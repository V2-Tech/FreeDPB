...
//?^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^/
DPB::DPB(gpio_num_t esc_gpio_num, gpio_num_t ir_gpio_num)
{
    ...
    _motor = new Motor(esc_gpio_num, DSHOT300);
    _rpmSensor = new RPMTracker(ir_gpio_num);

    _accel = new BMX055;
    _xShared.setDeviceModel(MODEL_BMX055);
}

int16_t DPB::init(QueueHandle_t xQueueSysInput_handle, QueueHandle_t xQueueSysOutput_handle, DPBShared *sharedData)
{
    if ((xQueueSysInput_handle == 0) || (xQueueSysOutput_handle == 0) || (sharedData == 0))
    {
        return ESP_FAIL;
    }
    ...
    ESP_LOGI(TAG, "Init accelerometer...");
    _init_accel();

    ESP_LOGI(TAG, "ESC arming sequence started...");
    _init_esc();

    ESP_LOGI(TAG, "Init IR sensor...");
    _init_rpm();

    ESP_LOGI(TAG, "Wait until all has been initialized");
    /* Wait until all has been initialized */

    while (_xShared.getInitStatus() != (READ_SETTINGS_DONE | INIT_ACCEL_DONE | INIT_ESC_DONE | INIT_RPM_S_DONE))
        ;
...
void DPB::_init_esc()
{
    ...
    _xShared.addInitStatus(INIT_ESC_DONE);
}

void DPB::_init_rpm()
{
    if (_rpmSensor->init(sensorTaskHandle, DEFAULT_PROP_NUM) == ESP_OK)
    {
        _xShared.addInitStatus(INIT_RPM_S_DONE);
    }
}

void DPB::_init_accel()
{
    accel_settings_t default_settings;
    int32_t spi_speed = _xShared.getDeviceModel() == MODEL_BMX055 ? SPI_SPEED_BMX055 : SPI_SPEED_ADXL345;

    if (_accel->init(HSPI_HOST, ACC_SPI_MISO, ACC_SPI_MOSI, ACC_SPI_SCLK, ACC_SPI_CS, spi_speed) != ESP_OK)
    {
        // ToDo: implement error message in loading screen
    }
    if (_accel->set_default_config() != ESP_OK)
    {
        // ToDo: implement error message in loading screen
    }

    if (_accel->set_range_reg((uint8_t)_xShared.getRange()) != ESP_OK)
    {
        // ToDo: implement error message in loading screen
    }

    _accel->get_acc_settings(default_settings);

    _xShared.setBandWidth(default_settings.band);
    _xShared.setRange(default_settings.range);

    _xShared.addInitStatus(INIT_ACCEL_DONE);
}
...
{
    _rpmSensor->loop();
    _xShared.setRPM(_rpmSensor->get_rpm());
}

void DPB::loop_accel(void)
{
    acc_packet_t acc_data;
    size_t i;
    ...
    _rpmSensor->get_rotation_done(); //? call to clear dummy set of bit
    while (i < ACC_DATA_BUFFER_SIZE)
    {
        if (_accel->read_accelerations_packet(acc_data) == ESP_OK)
        {
            uint8_t r_d;
            uint64_t time;

            _rpmSensor->get_time(time);
            r_d = _rpmSensor->get_rotation_done();
...
void DPB::exe(command_data_t command)
{
    ...
    case MOTOR_CMD:
        _motor->set_throttle(command.value.ull);
        break;
...
{
    _rpmSensor->reset_rpm_cnt();
    _motor->set_throttle(_xShared.getMeasureThrottle());
...
void DPB::_reset(void)
{
    ...
    _motor->set_throttle(0);
    _rpmSensor->stop_rpm_cnt();
...
void DPB::_exe_get_settings(sys_command_e request)
{
    ...
    case ACCEL_GET_BW_CMD:
        _accel->get_acc_settings(acc_settings);
...
void DPB::_exe_set_settings(command_data_t commnad)
{
    ...
    {
    case APP_SET_ACC_MODEL_CMD:
        _xShared.setDeviceModel((acc_model_e)commnad.value.ull);
        break;
    ...
    case ACCEL_SET_RANGE_CMD:
        if (_accel->set_range_reg((uint8_t)(1U << (commnad.value.ull + 1))) == ESP_OK)
        {
...
void DPB::_settings_read(void)
{
    ...
    if (err == ESP_OK)
    {
        ...
        ESP_LOGI(TAG, "User settings read");
    }
    else
    {
    ...
    _xShared.addInitStatus(READ_SETTINGS_DONE);
...
void DPB::__initNVS(void)
{
    ...
    if (err == ESP_ERR_NVS_NO_FREE_PAGES || err == ESP_ERR_NVS_NEW_VERSION_FOUND)
    {
        ...
        ESP_LOGE(TAG, "NVS user_data partition was truncated and needs to be erased");
