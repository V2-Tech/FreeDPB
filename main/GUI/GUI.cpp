...
void accel_model_settings_event_cb(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_VALUE_CHANGED)
    {
        gui_accelModel = lv_dropdown_get_selected(obj);
    }
}
...
void _exe_settings_update(command_data_t command)
{
    ...
    case APP_GET_ACC_MODEL_CMD:
        lv_dropdown_set_selected(gui_accel_model_dropdown, (uint16_t)command.value.ull);
        gui_accelModel = (int32_t)command.value.ll;
        break;
...
void _ask_settings_save(void)
{
    ...
    //* Accelerometer model
    command.command = APP_SET_ACC_MODEL_CMD;
    command.value.ll = (int64_t)gui_accelModel;
    xQueueSend(_xQueueCom2Sys, &command, portMAX_DELAY);
...
void _create_pages_settings(void)
{
    ...
    lv_label_set_text(_label, "Accel\nmodel");
    ...
    // Dropdown button
    gui_accel_model_dropdown = lv_dropdown_create(_tab);
    lv_obj_set_width(gui_accel_model_dropdown, 100);
    lv_obj_set_height(gui_accel_model_dropdown, LV_SIZE_CONTENT);
    lv_obj_set_align(gui_accel_model_dropdown, LV_ALIGN_CENTER);
    lv_obj_add_flag(gui_accel_model_dropdown, LV_OBJ_FLAG_SCROLL_ON_FOCUS);
    lv_dropdown_set_options(gui_accel_model_dropdown, "BMX055\nADXL345\nLSM6DS3");
    lv_obj_set_style_bg_color(gui_accel_model_dropdown, SECONDARY_BACKGROUND_COLOR, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(gui_accel_model_dropdown, LV_OPA_COVER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_opa(gui_accel_model_dropdown, LV_OPA_TRANSP, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(gui_accel_model_dropdown, DEFAULT_ELEMENT_ACCENT_COLOR, LV_PART_MAIN | LV_STATE_DEFAULT);

    // Dropdown list
    lv_obj_t *_list = lv_dropdown_get_list(gui_accel_model_dropdown);
    ...
    // Add callback
    lv_obj_add_event_cb(gui_accel_model_dropdown, accel_model_settings_event_cb, LV_EVENT_ALL, NULL);
    ...
    // Dropdown list
    _list = lv_dropdown_get_list(gui_unbalance_source_dropdown);
    lv_obj_set_style_bg_color(_list, SECONDARY_BACKGROUND_COLOR, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(_list, LV_OPA_COVER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_opa(_list, LV_OPA_TRANSP, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(_list, SECONDARY_ELEMENT_ACCENT_COLOR, LV_PART_SELECTED | LV_STATE_CHECKED);
